...
class AccountRateLimiterTest {
    ...
    fun credentials_share_a_bucket_and_limit_before_json_or_pbkdf2_work() {
        ...
        val limiter = AccountRateLimiter(
        ...
        testApplication {
            ...
            val limited = client.post("/api/v1/auth/register") {
            ...
            val otherIp = client.post("/api/v1/auth/login") {
            ...
            nowEpochMs += 60_000L
            val afterWindow = client.post("/api/v1/auth/login") {
                secureJsonFrom("198.51.100.7", "{")
            }
    ...
    @Test
    fun refresh_uses_an_independent_wider_bucket() = testApplication {
        val limiter = AccountRateLimiter(
        ...
        val limited = client.post("/api/v1/auth/refresh") {
    ...
    fun loopback_proxy_requires_one_ip_literal_and_public_peers_cannot_spoof_identity() =
        testApplication {
            ...
            val commaChain = client.post("/api/v1/auth/login") {
                secureJsonFrom(
            ...
            val invalidHost = client.post("/api/v1/auth/login") {
                secureJsonFrom(
    ...
    fun limiter_bounds_memory_without_evicting_live_buckets_and_cleans_expired_entries() {
        ...
        val atCapacity = assertIs<RateLimitDecision.Limited>(
...
private fun HttpRequestBuilder.secureJsonFrom(clientIp: String, body: String) {
...
private suspend fun io.ktor.client.statement.HttpResponse.errorCode(): String = bodyAsText()
