...
class AccountEndpointProtectionTest {
    @Test
    fun logout_and_profile_endpoints_have_independent_per_ip_limits() = testApplication {
    ...
    fun maximum_ciphertext_round_trips_within_derived_body_limit_and_one_extra_byte_is_rejected() =
        testApplication {
            ...
            val maximumRequest = protectedSyncBody(
                baseVersion = 0,
                nonceByte = 1,
            ...
            val uploaded = client.put("/api/v1/account/sync") {
            ...
            val rejected = client.put("/api/v1/account/sync") {
...
private fun HttpRequestBuilder.protectedJsonFrom(clientIp: String, body: String) {
...
private fun HttpRequestBuilder.protectedBearerFrom(clientIp: String, token: String) {
    header("X-Forwarded-Proto", "https")
    header("X-Forwarded-For", clientIp)
...
private fun protectedBase64(bytes: ByteArray): String =
