...
同一账号可以在不同设备同时加入同一房间，例如手机创建房间担任房主，平板输入房间码
作为观众。每台设备使用本机独立的 `clientId`、重连凭据和角色；账号相同不会自动授予
房主或管理员权限。同一设备的有效重连替换旧连接，其他设备继续在线。在线上限按设备
计数，离线设备在房间满员时需要等待空位。
...
- 移出其他账号时，该账号在房间中的所有设备一起移出；移出房主自己账号下的平板时，
  仅移出指定设备，不影响手机房主及其重连。
