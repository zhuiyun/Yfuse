...
async function renderPage(url, requestSignal) {
  ...
  return scheduler.run((signal) => {
    ...
    return withRenderContext(browser, contextOptions, signal, async (context) => {
...
const server = http.createServer(async (request, response) => {
  if (request.method === "GET" && request.url === "/health") {
    ...
    return jsonResponse(response, healthy ? 200 : 503, {
      status: healthy ? "ok" : "browser_unavailable", activePages: scheduler.active,
      queuedPages: scheduler.queue.length, cachedPages: cache.size,
    });
  }
  ...
  try {
    ...
    const html = await renderPage(payload.url, lifetime.signal);
    ...
    const status = code === "request_too_large" ? 413
      : ["weibo_session_required", "renderer_busy"].includes(code) ? 503
      : code === "render_timeout" ? 504 : 502;
