...
& .\gradlew.bat :phoneShared:testAndroidHostTest --tests 'com.yfuse.update.*' :composeApp:compileDebugKotlin `
    ...
    --offline --console=plain --no-daemon --max-workers=1 --stacktrace > (Join-Path $PSScriptRoot 'verify.log') 2>&1
$result = $LASTEXITCODE
Get-Content (Join-Path $PSScriptRoot 'verify.log') -Tail 45
exit $result
