...
[string[]]$tasks = @(switch ($Mode) {
    ...
    all { @(':phoneShared:testAndroidHostTest', ':tvShared:testAndroidHostTest', ':composeApp:compileDebugKotlin', ':tvApp:compileDebugKotlin', ':composeApp:lintDebug', ':tvApp:lintDebug', ':composeApp:verifyDesignSystemUsage', '--continue') }
})
$sourceBefore = @{}
