...
$stageRoot = Join-Path $taskRoot 'audit/playback-followup-fix-20260920/format-stage'
$sourcePaths = @(Get-Content audit/playback-followup-fix-20260920/format-paths.json -Raw | ConvertFrom-Json)
...
& 'C:\Users\app-inkbird\.jdks\ms-21.0.9\bin\java.exe' -cp ($jars -join ';') com.pinterest.ktlint.Main @formatFlags @staged > audit/playback-followup-fix-20260920/format-files.log 2>&1
$formatResult = $LASTEXITCODE
if ($formatResult -ne 0 -and -not $Check) {
    & 'C:\Users\app-inkbird\.jdks\ms-21.0.9\bin\java.exe' -cp ($jars -join ';') com.pinterest.ktlint.Main --format @staged > audit/playback-followup-fix-20260920/format-files.log 2>&1
    $formatResult = $LASTEXITCODE
}
Get-Content audit/playback-followup-fix-20260920/format-files.log -Tail 45
