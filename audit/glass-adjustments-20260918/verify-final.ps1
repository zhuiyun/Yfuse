...
& .\gradlew.bat :phoneShared:compileAndroidMain :tvShared:compileAndroidMain :composeApp:verifyDesignSystemUsage `
    ...
    --offline --console=plain --no-daemon --max-workers=1 > (Join-Path $PSScriptRoot 'verify-final.log') 2>&1
$result = $LASTEXITCODE
Get-Content (Join-Path $PSScriptRoot 'verify-final.log') -Tail 40
exit $result
