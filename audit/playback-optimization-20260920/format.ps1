...
$stageRoot = Join-Path $taskRoot 'audit/playback-optimization-20260920/format-stage'
$sourcePaths = @(Get-Content audit/playback-optimization-20260920/format-paths.json -Raw | ConvertFrom-Json)
...
$formatFlags = if ($Check) { @() } else { @('--format') }
& 'C:\Users\app-inkbird\.jdks\ms-21.0.9\bin\java.exe' -cp ($jars -join ';') com.pinterest.ktlint.Main @formatFlags @staged > audit/playback-optimization-20260920/format-files.log 2>&1
$formatResult = $LASTEXITCODE
if ($formatResult -ne 0 -and -not $Check) {
    & 'C:\Users\app-inkbird\.jdks\ms-21.0.9\bin\java.exe' -cp ($jars -join ';') com.pinterest.ktlint.Main --format @staged > audit/playback-optimization-20260920/format-files.log 2>&1
    $formatResult = $LASTEXITCODE
}
Get-Content audit/playback-optimization-20260920/format-files.log -Tail 45
if ($formatResult -ne 0) { exit $formatResult }
if ($Check) { Write-Output "Checked $($sourcePaths.Count) scoped Kotlin files."; exit 0 }
foreach ($relative in $sourcePaths) {
    ...
    if ((Get-FileHash -LiteralPath $target).Hash -ne $originalHashes[$relative]) { throw "Concurrent edit detected; source retained: $relative" }
