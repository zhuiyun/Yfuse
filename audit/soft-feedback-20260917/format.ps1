...
$stageRoot = Join-Path $taskRoot 'audit/soft-feedback-20260917/format-stage'
$baselineSource = Join-Path $taskRoot 'audit/soft-feedback-20260917/baseline/composeApp/src'
$sourcePaths = @(Get-ChildItem -LiteralPath $baselineSource -Recurse -Filter '*.kt' | ForEach-Object { 'composeApp/src/' + [IO.Path]::GetRelativePath($baselineSource, $_.FullName) })
$sourcePaths += @(
...
& 'C:\Users\app-inkbird\.jdks\ms-21.0.9\bin\java.exe' -cp ($jars -join ';') com.pinterest.ktlint.Main --format @staged > audit/soft-feedback-20260917/format-files.log 2>&1
$formatResult = $LASTEXITCODE
Get-Content audit/soft-feedback-20260917/format-files.log -Tail 45
