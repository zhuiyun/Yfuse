...
$stageRoot = Join-Path $taskRoot 'audit/disc-software-release-20260920/format-stage'
$sourcePaths = @(Get-Content audit/disc-software-release-20260920/format-paths.json -Raw | ConvertFrom-Json)
...
$staged = @(foreach ($relative in $sourcePaths) {
    ...
    if ((Get-FileHash -LiteralPath (Join-Path $taskRoot $relative)).Hash -ne $originalHashes[$relative]) { throw "Concurrent source edit while staging: $relative" }
    $target
})
Copy-Item -LiteralPath .editorconfig -Destination (Join-Path $stageRoot '.editorconfig')
& 'C:\Users\app-inkbird\.jdks\ms-21.0.9\bin\java.exe' -cp ($jars -join ';') com.pinterest.ktlint.Main --format @staged > audit/disc-software-release-20260920/format-files.log 2>&1
$formatResult = $LASTEXITCODE
if ($formatResult -ne 0) {
    & 'C:\Users\app-inkbird\.jdks\ms-21.0.9\bin\java.exe' -cp ($jars -join ';') com.pinterest.ktlint.Main --format @staged > audit/disc-software-release-20260920/format-files.log 2>&1
    $formatResult = $LASTEXITCODE
}
Get-Content audit/disc-software-release-20260920/format-files.log -Tail 45
...
foreach ($relative in $sourcePaths) {
    ...
    if ((Get-FileHash -LiteralPath $target).Hash -ne $originalHashes[$relative]) { throw "Concurrent source edit before writeback: $relative. Reread before retry." }
