...
data class AmbientLight(
    ...
    val isDark: Boolean
        get() = mean == Color.Black && left.all { it == Color.Black } && right.all { it == Color.Black } &&
            top.all { it == Color.Black } && bottom.all { it == Color.Black }
...
fun ambientLightDiffers(
    ...
    fun edgeDiffers(a: List<Color>, b: List<Color>): Boolean {
        for (index in a.indices) if (far(a[index], b[index])) return true
        return false
    }
    return far(current.mean, next.mean) || edgeDiffers(current.left, next.left) ||
        edgeDiffers(current.right, next.right) || edgeDiffers(current.top, next.top) ||
...
fun ambientLightAccent(light: AmbientLight): Color? =
    light.accent.takeIf { light.accentWeight > 0f }

/** Dark frames fade back to the artwork accent without starting another animation clock. */
fun ambientSeekAccent(light: AmbientLight?, fallback: Color): Color =
    if (light == null) fallback else interpolateArtworkPageColor(fallback, light.accent, light.accentWeight)
 ...
 */
@Composable
fun rememberAmbientLight(target: AmbientLight?, active: Boolean = true, animate: Boolean = true): State<AmbientLight> {
    ...
    val transition =
        remember(targetLight, reduceMotion, visible, active, animate) {
            ...
            AmbientLightTransition(start, targetLight, active && animate && !reduceMotion && visible && start != targetLight)
        }
    val output =
        remember(transition) {
...
fun ambientLightFalloffBrushes(picture: Rect, bounds: Size): List<Brush> {
    fun falloff(from: Offset, to: Offset) = Brush.linearGradient(
