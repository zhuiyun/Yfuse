...
class PlaybackSourceSwitchCoordinatorTest {
    ...
    fun observed_same_engine_media_round_trip_does_not_revive_an_old_switch() =
        runTest {
            val coordinator = PlaybackSourceSwitchCoordinator()
            val first = PlaybackState(currentIndex = 0)
            ...
            coordinator.observePlayback(original.engineIdentity, first.copy(currentIndex = 1))
            coordinator.observePlayback(original.engineIdentity, first)
            encoderStopped.complete(true)
    ...
    @Test
    fun seek_and_output_evidence_resets_do_not_supersede_a_same_item_switch() =
        runTest {
            val coordinator = PlaybackSourceSwitchCoordinator()
            var state = PlaybackState(currentIndex = 0, positionMs = 10_000L)
            coordinator.observePlayback(original.engineIdentity, state)
            val request = coordinator.begin(original)
            val encoderStopped = CompletableDeferred<Boolean>()
            val pending = async { coordinator.prepare(request, { original }) { encoderStopped.await() } }
            runCurrent()

            // MediaCodec flush, surface replacement and audio-route changes reset output evidence
            // while the current media request stays the same. The handover should use its live position.
            state =
                state.copy(
                    positionMs = 75_000L,
                    diagnostics =
                        state.diagnostics.copy(
                            outputEvidenceGeneration = state.diagnostics.outputEvidenceGeneration + 1,
                            outputEvidence =
                                state.diagnostics.outputEvidence.copy(
                                    sessionRevision = state.diagnostics.outputEvidence.sessionRevision + 1,
                                ),
                        ),
                )
            coordinator.observePlayback(original.engineIdentity, state)
            ...
            assertEquals(PlaybackSourceSwitchPreparation.Ready, pending.await())
        }
