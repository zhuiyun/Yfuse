...
$stageRoot = Join-Path $taskRoot 'audit/health-fixes-20260920/core2-resources-format-stage'
$sourcePaths = @(Get-Content audit/health-fixes-20260920/core2-resources-format-paths.json -Raw | ConvertFrom-Json)
...
& 'C:\Users\app-inkbird\.jdks\ms-21.0.9\bin\java.exe' -cp ($jars -join ';') com.pinterest.ktlint.Main --format @staged > audit/health-fixes-20260920/core2-resources-format-files.log 2>&1
$formatResult = $LASTEXITCODE
if ($formatResult -ne 0) {
    & 'C:\Users\app-inkbird\.jdks\ms-21.0.9\bin\java.exe' -cp ($jars -join ';') com.pinterest.ktlint.Main --format @staged > audit/health-fixes-20260920/core2-resources-format-files.log 2>&1
    $formatResult = $LASTEXITCODE
}
Get-Content audit/health-fixes-20260920/core2-resources-format-files.log -Tail 45
