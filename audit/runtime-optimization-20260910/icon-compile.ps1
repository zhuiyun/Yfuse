...
$reviewArguments = @(
    ':composeApp:compileReleaseKotlinAndroid', ':tvApp:compileDebugKotlinAndroid',
...
& $gradle @reviewArguments 2>&1 | Tee-Object -FilePath (Join-Path $PSScriptRoot 'icon-compile.log')
if ($LASTEXITCODE -ne 0) { throw 'Workspace review checks failed; inspect icon-compile.log.' }
