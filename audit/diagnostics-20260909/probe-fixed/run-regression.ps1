...
$sourceRoot = Join-Path $RepositoryRoot 'composeApp\src\androidMain\kotlin\com\yfuse\core2\android'
$sources = @('AndroidProbeBudget.kt', 'AndroidProbeBudgetCoroutines.kt', 'AndroidBoundedProbe.kt', 'AndroidSerializedPlayerRelease.kt') | ForEach-Object { Join-Path $sourceRoot $_ }
$sources += Join-Path $RepositoryRoot 'composeApp\src\androidMain\kotlin\com\yfuse\core\playback\AndroidBlockingMediaProbeLane.kt'
$testRoot = Join-Path $RepositoryRoot 'composeApp\src\androidUnitTest\kotlin\com\yfuse\core2\android'
@('AndroidProbeBudgetTest.kt', 'AndroidBoundedProbeTest.kt') | ForEach-Object {
    Write-Utf8 $_ ([System.IO.File]::ReadAllText((Join-Path $testRoot $_)).Replace('import kotlin.test.Test', '').Replace('@Test', ''))
    $sources += Join-Path $outputDirectory $_
}
Write-Utf8 'AndroidBlockingMediaProbeLaneTest.kt' ([System.IO.File]::ReadAllText((Join-Path $RepositoryRoot 'composeApp\src\androidUnitTest\kotlin\com\yfuse\core\playback\AndroidBlockingMediaProbeLaneTest.kt')).Replace('import kotlin.test.Test', '').Replace('@Test', ''))
$sources += Join-Path $outputDirectory 'AndroidBlockingMediaProbeLaneTest.kt'
...
if ($LASTEXITCODE -ne 0) { throw 'Probe regression compilation failed.' }
$result = & java '-cp' (@($classesDirectory, $stdlib, $kotlinTest, $coroutines) -join [System.IO.Path]::PathSeparator) 'RunnerKt'
if ($LASTEXITCODE -ne 0) { throw 'Probe regression assertions failed.' }
