...
STAGE = ROOT / ".native-build/diagnostics-fix-20260909"
...
SOURCE_NAMES = ("ycore_demux_jni.cpp", "ycore_tone_map.h", "ycore_disc_uri.h", "ycore_overlay_plane.h")
...
def main() -> None:
    ...
    expected = next(line.split()[0] for line in (ROOT / "scripts/engine-checksums.sha256").read_text().splitlines() if line.endswith("  libmpv-release.aar"))
    ...
    archive_path = STAGE / "ffmpeg-b79d4c4.tar.gz"
    ...
    for name in ("nativeSubtitleDisplaySetApiVersion", "nativeOpenCancellable", "nativeCreateCancellation", "nativeRenderAss", "nativeDemuxHandleContractVersion"):
