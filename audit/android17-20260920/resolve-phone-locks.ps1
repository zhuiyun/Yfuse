...
& .\gradlew.bat :composeApp:resolveAndroid17Locks :phoneShared:resolveAndroid17Locks :macrobenchmark:resolveAndroid17Locks --update-locks 'io.ktor:*' -I audit/android17-20260920/resolve-tv-locks.init.gradle `
    ...
    --offline --console=plain --no-daemon --max-workers=1 > (Join-Path $PSScriptRoot 'resolve-phone-locks.log') 2>&1
$result = $LASTEXITCODE
Get-Content (Join-Path $PSScriptRoot 'resolve-phone-locks.log') -Tail 40
exit $result
