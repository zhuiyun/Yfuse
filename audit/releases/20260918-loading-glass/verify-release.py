...
OUT = ROOT / 'artifacts/releases/loading-glass-1.0.71-233'
APK = OUT / 'Yfuse-1.0.71-233-full-arm64-signed.apk'
PREVIOUS = ROOT / 'artifacts/releases/library-handoff-1.0.70-232/Yfuse-1.0.70-232-full-arm64-signed.apk'
...
require(previous.get('versionName') == '1.0.70' and previous.get('versionCode') == '232',
        'Expected verified latest delivered baseline 1.0.70 (232)')
...
require(current.get('versionName') == properties['VERSION_NAME'] == '1.0.71', 'Version name mismatch')
require(current.get('versionCode') == properties['VERSION_CODE'] == '233', 'Version code mismatch')
...
with zipfile.ZipFile(APK) as package, zipfile.ZipFile(PREVIOUS) as previous_package, \
    ...
    for name, expected_digest in baseline_native['installedLibraries'].items():
        ...
        require(package.read(entry) == previous_package.read(entry), f'Packaged native differs from actual 232 APK: {name}')
