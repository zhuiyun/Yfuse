...
def main():
    ...
    parser.add_argument("--previous-apk", type=Path, default=ROOT / (
        "artifacts/releases/playback-recovery-1.0.77-239/Yfuse-1.0.77-239-full-arm64-signed.apk"))
    ...
    require(previous.get("versionName") == "1.0.77" and previous.get("versionCode") == "239",
            "Expected verified latest delivered baseline 1.0.77 (239)")
    ...
    require(current.get("versionName") == properties.get("VERSION_NAME") == "1.0.78", "Version name mismatch")
    require(current.get("versionCode") == properties.get("VERSION_CODE") == "240", "Version code mismatch")
    ...
    with zipfile.ZipFile(apk) as package, zipfile.ZipFile(previous_apk) as baseline, \
        ...
        require(b"preserveUrlOnSessionRefresh" not in old_dex, "Unexpected baseline address-fix marker")
