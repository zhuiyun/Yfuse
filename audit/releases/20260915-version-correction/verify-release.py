...
APK = OUT / 'Yfuse-1.0.65-227-full-arm64-signed.apk'
PREVIOUS = AUDIT / 'previous-1.0.64-226/Yfuse-226-1.0.64-signed.apk'
...
require(previous.get('versionName') == '1.0.64' and previous.get('versionCode') == '226',
        'Expected verified latest delivered baseline 1.0.64 (226)')
...
require(current.get('versionName') == properties['VERSION_NAME'] == '1.0.65', 'Version name mismatch')
require(current.get('versionCode') == properties['VERSION_CODE'] == '227', 'Version code mismatch')
...
native = []
ycore = []
baseline_native = json.loads((AUDIT / 'native-baseline/installed-verification.json').read_text(encoding='utf-8'))
...
with zipfile.ZipFile(APK) as package, zipfile.ZipFile(PREVIOUS) as previous_package, \
    ...
    for name, expected_digest in baseline_native['installedLibraries'].items():
        ...
        require(package.read(entry) == previous_package.read(entry), f'Packaged native differs from actual 226 APK: {name}')
