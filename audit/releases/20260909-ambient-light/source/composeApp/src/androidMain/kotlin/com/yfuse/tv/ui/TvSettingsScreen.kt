...
internal fun TvSettingsScreen(
    ...
    when (page) {
        ...
        TvSettingsPage.MediaDiscovery ->
            TvMediaDiscoverySettingsPage(
                component = component,
                focusMemory = focusMemory,
                navigationRequester = navigationRequester,
                firstRowRequester = pageRequester,
...
private fun TvSettingsRootPage(
    ...
    TvSettingsPageScaffold(page = TvSettingsPage.Root) {
        ...
        if (query.isNotBlank()) {
            val matches = searchTvSettings(query, includeMediaDiscovery = account.canUseMediaDiscovery())
            if (matches.isEmpty()) {
        ...
        if (account.canUseMediaDiscovery()) {
            item(key = "settings-media-discovery") {
                TvSettingRow(
                    title = TvSettingsPage.MediaDiscovery.title,
                    value = "",
                    stableId = "settings:media-discovery",
                    focusMemory = focusMemory,
                    onClick = { onOpen(TvSettingsPage.MediaDiscovery) },
                    icon = AppIcons.Cloud,
                    focusScope = scope,
                    subtitle = TvSettingsPage.MediaDiscovery.subtitle,
                    navigationRequester = navigationRequester,
                )
            }
        }
...
private val tvSettingsKeywords: Map<TvSettingsPage, String> =
    mapOf(
        ...
        TvSettingsPage.MediaDiscovery to "资源 转存 站点 123 tgto",
    )

internal fun searchTvSettings(
    query: String,
    ...
    return TvSettingsPage.entries
        ...
        .filter { includeMediaDiscovery || it != TvSettingsPage.MediaDiscovery }
        .filter { page ->
