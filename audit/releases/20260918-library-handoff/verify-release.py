...
OUT = ROOT / 'artifacts/releases/library-handoff-1.0.70-232'
APK = OUT / 'Yfuse-1.0.70-232-full-arm64-signed.apk'
PREVIOUS = ROOT / 'artifacts/releases/soft-feedback-1.0.69-231/Yfuse-1.0.69-231-full-arm64-signed.apk'
...
require(previous.get('versionName') == '1.0.69' and previous.get('versionCode') == '231',
        'Expected verified latest delivered baseline 1.0.69 (231)')
...
require(current.get('versionName') == properties['VERSION_NAME'] == '1.0.70', 'Version name mismatch')
require(current.get('versionCode') == properties['VERSION_CODE'] == '232', 'Version code mismatch')
...
with zipfile.ZipFile(APK) as package, zipfile.ZipFile(PREVIOUS) as previous_package, \
    ...
    for name, expected_digest in baseline_native['installedLibraries'].items():
        ...
        require(package.read(entry) == previous_package.read(entry), f'Packaged native differs from actual 231 APK: {name}')
