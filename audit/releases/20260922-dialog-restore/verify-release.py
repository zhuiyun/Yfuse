...
def main():
    ...
    parser.add_argument("--apk", type=Path, default=ROOT / (
        "artifacts/releases/dialog-restore-1.0.79-241/Yfuse-1.0.79-241-full-arm64-signed.apk"))
    parser.add_argument("--previous-apk", type=Path, default=ROOT / (
        "artifacts/releases/playback-address-1.0.78-240/Yfuse-1.0.78-240-full-arm64-signed.apk"))
    ...
    require(previous.get("versionName") == "1.0.78" and previous.get("versionCode") == "240",
            "Expected verified latest delivered baseline 1.0.78 (240)")
    ...
    require(current.get("versionName") == properties.get("VERSION_NAME") == "1.0.79", "Version name mismatch")
    require(current.get("versionCode") == properties.get("VERSION_CODE") == "241", "Version code mismatch")
    ...
    with zipfile.ZipFile(apk) as package, zipfile.ZipFile(previous_apk) as baseline, \
        ...
        animation_source = (ROOT / "composeApp/src/commonMain/kotlin/com/yfuse/core/designsystem/DialogAnimation.kt").read_text(encoding="utf-8")
        animation_labels = re.findall(r'^    \w+\("([^"]+)"', animation_source, re.M)
        require(len(animation_labels) == 43, "Expected all 43 dialog animation styles in source")
        require(all(label.encode("utf-8") in dex for label in animation_labels),
                "Packaged DEX does not contain all 43 dialog animation labels")
        ...
        require(b"preserveUrlOnSessionRefresh" in old_dex, "Expected the verified 1.0.78 address-fix baseline")
        require(b"preserveUrlOnSessionRefresh" in dex, "Playback address fix absent from APK")
    ...
    report = {
        ...
        "dialogAnimationCount": len(animation_labels), "dialogAnimationLabels": animation_labels,
