...
def main():
    ...
    parser.add_argument("--previous-apk", type=Path, default=ROOT / (
        "artifacts/releases/kernel-fixes-1.0.74-236/Yfuse-1.0.74-236-full-arm64-signed.apk"))
    ...
    require(previous.get("versionName") == "1.0.74" and previous.get("versionCode") == "236",
            "Expected verified latest delivered baseline 1.0.74 (236)")
    ...
    require(current.get("versionName") == properties.get("VERSION_NAME") == "1.0.75", "Version name mismatch")
    require(current.get("versionCode") == properties.get("VERSION_CODE") == "237", "Version code mismatch")
    ...
    with zipfile.ZipFile(apk) as package, zipfile.ZipFile(previous_apk) as baseline, \
        ...
        require(b"RejectedNoKey" in old_dex, "Previous APK does not reproduce the missing-key rejection")
    ...
    report = {
        ...
        "certificateSha256": CERT, "signatureVerified": True, "signatureV2": True, "updateMissingKeyRejectionRemovedFromDex": True, "debuggable": False,
