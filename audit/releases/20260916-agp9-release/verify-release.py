...
OUT = ROOT / 'artifacts/releases/agp9-1.0.67-229'
APK = OUT / 'Yfuse-1.0.67-229-full-arm64-signed.apk'
PREVIOUS = ROOT / 'artifacts/releases/ui-settings-1.0.66-228/Yfuse-1.0.66-228-full-arm64-signed.apk'
...
require(previous.get('versionName') == '1.0.66' and previous.get('versionCode') == '228',
        'Expected verified latest delivered baseline 1.0.66 (228)')
...
require(current.get('versionName') == properties['VERSION_NAME'] == '1.0.67', 'Version name mismatch')
require(current.get('versionCode') == properties['VERSION_CODE'] == '229', 'Version code mismatch')
...
with zipfile.ZipFile(APK) as package, zipfile.ZipFile(PREVIOUS) as previous_package, \
    ...
    for name, expected_digest in baseline_native['installedLibraries'].items():
        ...
        require(package.read(entry) == previous_package.read(entry), f'Packaged native differs from actual 228 APK: {name}')
