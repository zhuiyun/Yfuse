...
OUT = ROOT / 'artifacts/releases/merged-1.0.68-230'
APK = OUT / 'Yfuse-1.0.68-230-full-arm64-signed.apk'
PREVIOUS = ROOT / 'artifacts/releases/agp9-1.0.67-229/Yfuse-1.0.67-229-full-arm64-signed.apk'
...
require(previous.get('versionName') == '1.0.67' and previous.get('versionCode') == '229',
        'Expected verified latest delivered baseline 1.0.67 (229)')
...
require(current.get('versionName') == properties['VERSION_NAME'] == '1.0.68', 'Version name mismatch')
require(current.get('versionCode') == properties['VERSION_CODE'] == '230', 'Version code mismatch')
...
with zipfile.ZipFile(APK) as package, zipfile.ZipFile(PREVIOUS) as previous_package, \
    ...
    for name, expected_digest in baseline_native['installedLibraries'].items():
        ...
        require(package.read(entry) == previous_package.read(entry), f'Packaged native differs from actual 229 APK: {name}')
