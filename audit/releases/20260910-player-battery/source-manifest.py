...
paths = []
for directory in ('composeApp/src', 'mdkAndroid/src', 'watchTogetherProtocol/src', 'gradle', 'composeApp/libs', 'mdkAndroid/libs'):
...
for name in ('composeApp/build.gradle.kts', 'composeApp/proguard-rules.pro',
             ...
             'build.gradle.kts', 'settings.gradle.kts', 'version.properties', 'release-notes.txt', 'gradle.properties'):
    path = ROOT / name
    if path.is_file():
        paths.append(path)
