...
OUT = ROOT / 'artifacts/releases/ui-settings-1.0.66-228'
APK = OUT / 'Yfuse-1.0.66-228-full-arm64-signed.apk'
PREVIOUS = ROOT / 'artifacts/releases/ambient-family-1.0.65-227/Yfuse-1.0.65-227-full-arm64-signed.apk'
...
require(previous.get('versionName') == '1.0.65' and previous.get('versionCode') == '227',
        'Expected verified latest delivered baseline 1.0.65 (227)')
...
require(current.get('versionName') == properties['VERSION_NAME'] == '1.0.66', 'Version name mismatch')
require(current.get('versionCode') == properties['VERSION_CODE'] == '228', 'Version code mismatch')
...
with zipfile.ZipFile(APK) as package, zipfile.ZipFile(PREVIOUS) as previous_package, \
    ...
    for name, expected_digest in baseline_native['installedLibraries'].items():
        ...
        require(package.read(entry) == previous_package.read(entry), f'Packaged native differs from actual 227 APK: {name}')
