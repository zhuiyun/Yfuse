...
p=Path('harmonyApp/entry/src/main/cpp/ypreferences.cpp')
p.write_text('''#include "ycore/ycore.h"
...
''',encoding='utf-8')
p=Path('harmonyApp/entry/src/main/cpp/CMakeLists.txt');s=p.read_text(encoding='utf-8').replace('    "${CMAKE_CURRENT_SOURCE_DIR}/ysecure_huks.cpp"','    "${CMAKE_CURRENT_SOURCE_DIR}/ysecure_huks.cpp"\n    "${CMAKE_CURRENT_SOURCE_DIR}/ypreferences.cpp"').replace('target_link_libraries(ycore PRIVATE libasset_ndk.z.so)','target_link_libraries(ycore PRIVATE libasset_ndk.z.so libohpreferences.so)');p.write_text(s,encoding='utf-8')
p=Path('harmonyApp/entry/src/main/cangjie/src/storage/native_preferences.cj')
