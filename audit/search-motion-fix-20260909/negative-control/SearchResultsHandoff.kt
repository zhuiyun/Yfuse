...
internal class SearchRevealOrder {
    ...
    fun progress(
        ...
        return searchRevealProgress(progress, index)
    }
}
...
internal fun rememberSearchResultsHandoff(
    ...
    LaunchedEffect(progress, enhanced) {
        if (progress.value < 1f) {
            val animation =
                if (layered) {
                    tween<Float>(680, easing = LinearEasing)
                } else {
    ...
    LaunchedEffect(waiting) {
        ...
        if (waiting) {
            while (true) {
                ...
                pulse.animateTo(1f, tween(850, easing = Motion.Curve))
                pulse.animateTo(0f, tween(850, easing = Motion.Curve))
                delay(16)
            }
        }
    }
    ...
    return remember(progress, layered, order, field, icon) { SearchRevealMotion(progress, layered, order, field, icon) }
}
