...
$stageRoot = Join-Path $taskRoot 'audit/kernel-fixes-20260920/ownership/format-stage'
$sourcePaths = @(Get-Content audit/kernel-fixes-20260920/ownership/format-paths.json -Raw | ConvertFrom-Json)
...
$sourceHashes = @{}; foreach ($relative in $sourcePaths) { $sourceHashes[$relative] = (Get-FileHash -LiteralPath (Join-Path $taskRoot $relative)).Hash }
...
& 'C:\Users\app-inkbird\.jdks\ms-21.0.9\bin\java.exe' -cp ($jars -join ';') com.pinterest.ktlint.Main --format @staged > audit/kernel-fixes-20260920/ownership/format-files.log 2>&1
$formatResult = $LASTEXITCODE
if ($formatResult -ne 0) {
    & 'C:\Users\app-inkbird\.jdks\ms-21.0.9\bin\java.exe' -cp ($jars -join ';') com.pinterest.ktlint.Main --format @staged > audit/kernel-fixes-20260920/ownership/format-files.log 2>&1
    $formatResult = $LASTEXITCODE
}
Get-Content audit/kernel-fixes-20260920/ownership/format-files.log -Tail 45
...
foreach ($relative in $sourcePaths) {
    ...
    if ((Get-FileHash -LiteralPath $target).Hash -ne $sourceHashes[$relative]) { throw "Source changed during formatting: $relative" }
