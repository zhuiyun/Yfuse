...
$stageRoot = Join-Path $taskRoot 'audit/kernel-fixes-20260920/client/format-stage'
$sourcePaths = @(Get-Content audit/kernel-fixes-20260920/client/format-paths.json -Raw | ConvertFrom-Json)
...
& 'C:\Users\app-inkbird\.jdks\ms-21.0.9\bin\java.exe' -cp ($jars -join ';') com.pinterest.ktlint.Main --format @staged > audit/kernel-fixes-20260920/client/format-files.log 2>&1
$formatResult = $LASTEXITCODE
if ($formatResult -ne 0) {
    & 'C:\Users\app-inkbird\.jdks\ms-21.0.9\bin\java.exe' -cp ($jars -join ';') com.pinterest.ktlint.Main --format @staged > audit/kernel-fixes-20260920/client/format-files.log 2>&1
    $formatResult = $LASTEXITCODE
}
Get-Content audit/kernel-fixes-20260920/client/format-files.log -Tail 45
