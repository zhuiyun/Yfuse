...
| 3 | Capability engine | `AndroidYCapabilityProvider` reads codec profiles/levels, size/rate, HDR display/audio/tunnel/secure/adaptive/low-latency facts. `AndroidCodecConfigurationProbe` actively creates/configures/starts an advertised decoder against a valid private Surface; `AndroidRuntimeCapabilityRegistry` distinguishes advertised, configured, rendered and rejected evidence without turning an inconclusive vendor/Surface failure into a false rejection. | `YCapabilityTest`, `AndroidRuntimeCapabilityRegistryTest`, runtime probe instrumentation |
...
| 18 | Network/cache | `YMediaTransport` has concrete OkHttp HTTP(S)/WebDAV Range, jcifs-ng SMB2/3 random-access and Cronet HTTP/2/HTTP/3 executors. `AndroidTransportMediaDataSource`, the remote-disc Range source, `YBufferController` and `YCachePlanner` provide incremental access, strict range validation, redirect/auth protection, credential-redacted identities and bitrate/memory-aware buffering. | `YMediaTransportTest`, `AndroidHttpMediaTransportTest`, `YBufferControllerTest`, `AndroidHttpRangeMediaDataSourceTest`, remote Range tests |
