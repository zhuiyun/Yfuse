...
| 12 | Audio engine | Native PCM decode and encoded `AudioTrack` passthrough routes cover AAC/FLAC/ALAC/MP3/Opus, AC3/EAC3/JOC, TrueHD/Atmos and DTS/DTS-HD/DTS:X capability families, with live device-route reevaluation. Passthrough rejection and non-1.0x speed hand over in place to MediaCodec PCM and then FFmpeg PCM when necessary. `nativeDualDolbyOutput` is derived only when verified native-DV video and verified encoded Atmos audio coexist in one session. | `AndroidEncodedAudioTrackRenderNodeTest`, `YAudioHandoverPolicyTest`, `YPlayerDiagnosticsTest`, strategy audio tests and dual-Dolby release-evidence self-test |
