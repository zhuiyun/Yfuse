...
| 8 | P7/FEL policy | P7 MEL, FEL and unknown enhancement-layer evidence are separate states. YCore records RPU/EL only after their access unit enters the exact Dolby decoder and records RPU applied only after verified native-DV output; EL delivery remains separate from FEL composition. A fail-closed vendor evidence provider is now the only code path that can promote delivered P7 EL to composed FEL. | P7 MEL/FEL/unknown truth cases, provider fail-closed tests, adapter evidence tests and release-evidence self-tests |
...
| 12 | Audio engine | Native PCM decode and encoded `AudioTrack` passthrough routes cover AAC/FLAC/ALAC/MP3/Opus, AC3/EAC3/JOC, TrueHD/Atmos and DTS/DTS-HD/DTS:X capability families, with live device-route reevaluation. Passthrough rejection and non-1.0x speed hand over in place to MediaCodec PCM and then FFmpeg PCM when necessary. `nativeDualDolbyOutput` is generation-scoped and derived only when verified native-DV video and verified encoded Atmos audio coexist after the latest source/seek/Surface/route/DRM event. TrueHD Atmos promotion additionally requires the fail-closed receiver evidence provider. | `AndroidAudioRouteEvidenceTest`, `YDualDolbyEvidenceTest`, `YPlayerDiagnosticsTest`, strategy audio tests and dual-Dolby release-evidence self-test |
...
| 16 | ISO/BDMV | Disc descriptors, native remote ISO Range source and libbluray streams enter `AndroidYCoreDiscRouteFactory` first. Disc API v2 owns title/chapter/angle plus HDMV IG overlay, menu events, D-pad/back and touch input. mpv remains source-creation fallback; BD-J stays disabled behind its isolated provider contract. | native NDK link, optical playback, HDMV input/overlay, disc inspector, remote preflight and fallback tests |
