...
| P1-10 | 标题下方新增只读状态行：已收藏、稍后观看、已看完、个人收藏、想看。没有状态时不占位置；点击打开“更多操作”，读屏会读出各个状态。顶栏“…”左边新增收藏键，带选中状态描述和触感反馈；Plex 不支持收藏，不显示这个键。 | `DetailHero`、`DetailScreen` |
...
| P1-12 | 当前影片所在的房间，只打开分享面板，不再解散重建。房间影片未知时，也只分享。别的影片的房间，先确认再新建，文案区分房主和访客。“继续分享邀请”只对当前影片所在的房间显示。 | `DetailScreen.watchRoomAction` |
| P1-13 | 加入下载后会提示实际加入了几集、几集因版本不符被跳过，以及是否在等 Wi-Fi。离线播放从上次的位置继续；看完 95% 以上或已标记为看过时，从头播放。 | `DetailComponent.download`（返回 `OfflineEnqueueResult`）、`OfflineMedia.offlineStartPositionMs`、`ProfileScreen` |
...
| P2-23 | 整部剧和批量标记已看或未看之前，都要先确认（写明季数，并说明续播进度会一起清除）。标记后，选集和播放键立即刷新，包括不在当前季的播放目标。“重置”按钮已删除：仓库里没有“只清续播点”的接口，所以没有新造接口。 | `DetailExecutor`、`DetailReducer.SeriesProgressChanged`、`EpisodeProgressManager` |
...
| P2-27 | 外部链接按类型拼接：剧集用 `/tv/`，电影用 `/movie/`；TheTVDB 分别用 series、movie、episode。单集只有拿到剧集的 TMDB id 时才显示 TMDB 链接。 | `DetailSections` |
