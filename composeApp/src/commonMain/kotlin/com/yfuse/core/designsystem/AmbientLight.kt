 ...
 *
 * Every 250ms the player reads a 32×18 thumbnail of the frame. The two pixel rows hugging each
...
data class AmbientLight(
    ...
    val isDark: Boolean
        get() = mean == Color.Black && (left + right + top + bottom).all { it == Color.Black }
...
fun lerpAmbientLight(
    ...
    fun mix(
        a: List<Color>,
        b: List<Color>,
    ) = a.zip(b) { from, to -> interpolateArtworkPageColor(from, to, fraction) }
    return AmbientLight(
 ...
 * would otherwise restart the 600ms fade four times a second and the light would never settle.
 */
fun ambientLightDiffers(
    ...
    return (current.left + current.right + current.top + current.bottom + current.mean)
        .zip(next.left + next.right + next.top + next.bottom + next.mean)
        .any { (a, b) -> far(a, b) }
}
 ...
 */
fun ambientLightAccent(light: AmbientLight): Color? =
    light.mean.takeUnless { it == Color.Black }?.let { harmonizeArtworkAccent(it, darkTheme = true) }
 ...
 */
@Composable
fun rememberAmbientLight(target: AmbientLight?): State<AmbientLight> {
    val reduceMotion = LocalAccessibilityOptions.current.reduceMotion
    val visible = LocalRouteVisible.current
    val targetLight = target ?: AmbientLight.Off
    val previousOutput = remember { arrayOfNulls<State<AmbientLight>>(1) }
    val transition =
        remember(targetLight, reduceMotion, visible) {
            ...
            AmbientLightTransition(start, targetLight, !reduceMotion && visible && start != targetLight)
        }
    val output =
        remember(transition) {
    ...
    LaunchedEffect(transition) {
        if (transition.animate) {
            animate(0f, 1f, animationSpec = tween(AMBIENT_LIGHT_FADE_MS, easing = Motion.Curve)) { value, _ ->
                output.value = lerpAmbientLight(transition.start, transition.target, value)
            }
...
fun DrawScope.drawAmbientLight(
    ...
    clipRect(excluded.left, excluded.top, excluded.right, excluded.bottom, ClipOp.Difference) {
        ...
        fun bar(
            ...
            drawRect(falloff(edge, screen), rect.topLeft, rect.size)
        }
        ...
        if (picture.right < bounds.width - 0.5f) {
            bar(
                ...
                edge = Offset(picture.right, 0f),
                screen = Offset(bounds.width, 0f),
            )
        }
        if (picture.top > 0.5f) {
            bar(
        ...
        if (picture.bottom < bounds.height - 0.5f) {
            bar(
                ...
                edge = Offset(0f, picture.bottom),
                screen = Offset(0f, bounds.height),
            )
        }
    }
}
