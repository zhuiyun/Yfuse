...
private data class PreparedVideoAccessUnit(
    val data: ByteArray,
    val hdr10PlusPayload: ByteArray?,
)
...
internal class AndroidEnhancedPlaybackSession(
    context: Context,
    @Volatile private var demuxer: YDemuxer = AndroidFfmpegDemuxer(),
    ...
    private val subtitleCues = YSubtitleCueBuffer()
    private var secondarySubtitleTrack: YDemuxTrack? = null
    private val secondarySubtitleCues = YSubtitleCueBuffer()
    private var audioOutputPath = YAudioOutputPath.None
    private var surface: Surface? = null
    private val pendingSamples = linkedMapOf<YTrackId, YQueuedDemuxResult.Sample>()
    private val blockedInputTracks = mutableSetOf<YTrackId>()
    private var inputGeneration = 0L
    private val videoAccessUnits = AndroidAccessUnitCache<YCompressedSample, PreparedVideoAccessUnit>()
    ...
    fun open(
        ...
        val adoptedOpen =
            if (preparedDemux != null) {
                ...
                demuxer = preparedDemux.demuxer
                demuxReadAhead = AndroidDemuxReadAheadNode(demuxer)
                // Establish ownership before validating the (revocable) Surface so failure cleanup
                // also closes a transferred native context when output disappears during handoff.
                demuxReadAhead.adoptOpen(preparedDemux.openResult)
            } else {
                null
            }
        ...
        val audioSelection =
            selectEnhancedAudioTrack(
                ...
                preference = initialTrackSelection?.audio,
            )
        val audioTrack = audioSelection?.track
        val initialSubtitleTrack =
            if (initialTrackSelection?.subtitlesDisabled == true) {
                null
            } else {
                result.tracks
                    .preferredSubtitleTrack(initialTrackSelection?.subtitle)
                    ?.takeIf {
                        it.subtitle?.format?.let { format ->
                            format.textOverlaySupported || demuxReadAhead.supportsSubtitleFormat(format)
                        } == true
                    }
            }
        ...
        this.subtitleTrack = initialSubtitleTrack
        if (audioTrack == null) {
    ...
    fun setOutputSurface(next: Surface) {
        ...
        surface = next
        if (!softwareVideoActive && gpuVideoOutput == null) attachVideoRenderEvidence()
    }
    ...
    private fun applyAudioTrack(
        ...
        softwareAudioActive = nextUsesSoftware
        captureAudioRoutingGeneration()
        seekToInternal(positionUs, tailRetry = false, resetVideoDecoder = false, selectedTracks = selectedTrackIds())
    ...
    private fun seekToInternal(
        ...
        videoOutputEpoch.reset()
        if (playing) pausedPreview.clear() else pausedPreview.begin(target)
        ...
        gpuVideoOutput?.let { gpu ->
            check(gpu.resetOutputEvidence { replacement -> videoDecoder.setOutputSurface(replacement) }) {
                "Vulkan seek output generation reset failed"
            }
        }
        subtitleCues.clear()
        secondarySubtitleCues.clear()
    ...
    private fun pumpInternal(): Boolean {
        ...
        if (playing && !refreshOutputGate()) return didWork
        ...
        if (pausedPreview.active && (gpuVideoOutput?.outputVerified ?: firstVideoFrameRendered)) {
            pausedPreview.frameRendered()
        }
    ...
    fun snapshot(): YEnhancedPlaybackSnapshot {
        collectSoftwareRenderProgress()
        val subtitlePositionUs = currentPositionUs()
        subtitleCues.prune(subtitlePositionUs - SUBTITLE_HISTORY_US, subtitlePositionUs)
        secondarySubtitleCues.prune(subtitlePositionUs - SUBTITLE_HISTORY_US, subtitlePositionUs)
        val subtitleActiveSecondary = secondarySubtitleCues.activeDisplayCount(subtitlePositionUs)
        val subtitleRetainedCount = subtitleCues.retainedCueCount + secondarySubtitleCues.retainedCueCount
        val subtitleRetainedBytes = subtitleCues.retainedBitmapBytes + secondarySubtitleCues.retainedBitmapBytes
        val subtitleDroppedFuture =
            subtitleCues.droppedFutureDisplayCount + secondarySubtitleCues.droppedFutureDisplayCount
        val gpu = gpuVideoOutput
        val readAhead = demuxReadAhead.snapshot()
        val videoVerified = gpu?.outputVerified ?: firstVideoFrameRendered
        if (videoVerified && pausedPreview.resumePending) pausedPreview.frameRendered()
        if (gpu != null && videoVerified) firstVideoFrameRendered = true
        ...
        val outputEnded = ended()
        if (prepared && playing && outputEnded) pauseAtEnd()
        val renderedFrameRate =
            if (playing && videoVerified && !outputEnded) {
                val count =
                    when {
                        gpu != null -> gpu.renderedFrameCount.toLong()
                        softwareVideoActive -> softwareVideoRenderer.snapshot().renderedFrameCount.toLong()
                        else -> videoOutputEpoch.renderedFrameCount
                    }
                renderedFrameRateSampler.sample(count, SystemClock.elapsedRealtime())
            } else {
                renderedFrameRateSampler.reset()
                null
            }
        return YEnhancedPlaybackSnapshot(
            ...
            playing = playing && outputActive && !outputEnded,
            buffering = playing && (!outputActive || !videoVerified) && !outputEnded,
            ended = outputEnded,
            ...
            audioOutputFingerprint =
                if (passthrough) encodedAudioRenderer.audioRouteFingerprint else audioRenderer.audioRouteFingerprint,
            ...
            subtitleDiagnostics =
                mapOf(
                    "primaryFormat" to (subtitleTrack?.subtitle?.format?.name ?: "none"),
                    "secondaryFormat" to (secondarySubtitleTrack?.subtitle?.format?.name ?: "none"),
                    "bitmapDisplaySetApi" to if (FfmpegNativeBridge.subtitleDisplaySetAvailable) "2" else "unavailable",
                    "primaryReplaceCount" to subtitleCues.replacementCount.toString(),
                    "primaryClearCount" to subtitleCues.clearCount.toString(),
                    "secondaryReplaceCount" to secondarySubtitleCues.replacementCount.toString(),
                    "secondaryClearCount" to secondarySubtitleCues.clearCount.toString(),
                    "primaryActiveDisplays" to subtitleCues.activeDisplayCount(subtitlePositionUs).toString(),
                    "secondaryActiveDisplays" to subtitleActiveSecondary.toString(),
                    "retainedCueCount" to subtitleRetainedCount.toString(),
                    "retainedBitmapBytes" to subtitleRetainedBytes.toString(),
                    "droppedFutureDisplays" to subtitleDroppedFuture.toString(),
                ),
    ...
    /** May be called during open/release from another thread without touching codec ownership. */
    fun cancelPendingRead() {
        (demuxer as? AndroidFfmpegDemuxer)?.cancelPendingRead()
    }

    fun clearProbeDeadline() {
        (demuxer as? AndroidFfmpegDemuxer)?.clearProbeDeadline()
    }
    ...
    fun close() {
        ...
        videoOutputEpoch.reset()
        pausedPreview.clear()
        demuxReadAhead.awaitReleaseBarrier()
    ...
    private fun feedInput(): Boolean {
        ...
        var didWork = false
        val blockedTracks = blockedInputTracks.also { it.clear() }
        // Each track may retain at most one unaccepted access unit. Keep per-track ordering while
        // letting audio advance when video is full (and vice versa), including partial passthrough.
        for ((track, queued) in pendingSamples.toMap()) {
            val accepted = queueSample(queued)
            if (generation != inputGeneration) return true
            if (accepted) {
                pendingSamples.remove(track)
                didWork = true
            } else {
                blockedTracks += track
            }
        ...
        repeat(4) {
            val queued =
                when (val next = demuxReadAhead.pollSample(blockedTracks)) {
                    is YQueuedDemuxResult.Sample -> next
                    ...
                    YQueuedDemuxResult.EndOfInput -> {
                        if (pendingSamples.isEmpty()) inputEnded = true
                        return didWork || inputEnded
                    }
                }
            val accepted = queueSample(queued)
            if (generation != inputGeneration) return true
            if (accepted) {
                ...
                pendingSamples[queued.value.trackId] = queued
                blockedTracks += queued.value.trackId
            }
        }
        return didWork
    }

    private fun queueSample(queuedSample: YQueuedDemuxResult.Sample): Boolean {
        val generation = inputGeneration
        val sample = queuedSample.value
        ...
        val queued =
            when (sample.trackId) {
                    ...
                    if (softwareVideoActive) {
                        val softwareSample =
                            if (requireNotNull(plan).usesHdrFallback) {
                                sample.copy(data = preparedVideoAccessUnit(sample).data)
                            } else {
                                sample
                            }
                        yPlaybackStage(
                        ...
                        yPlaybackStage(
                            ...
                            videoDecoder.queueAccessUnit(
                                data = java.nio.ByteBuffer.wrap(preparedUnit.data),
                ...
                audioTrack?.id -> if (pausedPreview.active) YCodecQueueResult.Queued else queueAudioSample(sample)
                subtitleTrack?.id, secondarySubtitleTrack?.id -> {
                    queueSubtitleSample(sample, queuedSample.subtitleResult)
    ...
    private fun preparedVideoAccessUnit(sample: YCompressedSample): PreparedVideoAccessUnit =
        videoAccessUnits.getOrPrepare(sample) {
            yPlaybackStage(
                category = YPlaybackFailureCategory.Container,
                stage = YPlaybackFailureStage.Bitstream,
                safeDetail = "Enhanced video bitstream normalization",
            ) {
                val video = sourceVideoTrack?.video
                PreparedVideoAccessUnit(
    ...
    private fun decoderEnded(): Boolean =
        if (!softwareVideoActive && plan?.renderPath == YRenderPath.SurfaceDirect) {
            surfaceCompletion.ended(
                ...
                audioOutputEnded = audioTrack == null || audioOutputEnded,
                pausedPreviewPending = pausedPreview.resumePending,
            )
        } else {
            videoOutputEnded &&
                (audioTrack == null || audioOutputEnded) &&
                !pausedPreview.resumePending &&
                (gpuVideoOutput?.outputVerified ?: firstVideoFrameRendered)
        }
    ...
    private fun queueSubtitleSample(
        ...
        decodedResult: YSubtitleDecodeResult?,
    ) {
        val track =
        ...
        val positionUs = currentPositionUs()
        cues.prune(positionUs - SUBTITLE_HISTORY_US, positionUs)
        if (decodedResult != null) {
            cues.apply(decodedResult)
    ...
    private fun attachVideoRenderEvidence() {
        ...
        videoDecoder.setOnFrameRenderedListener { presentationTimeUs, realtimeNs ->
            ...
            val audioClock =
                if (videoOutputEpoch.recordRenderTime(generation, realtimeNs)) audioClockSnapshot() else null
            lastAvSyncOffsetUs =
