...
/**
 * 动画2 — the drop pools in a sunken well, boils into bubbles, and blows out as spray.
 *
 * Where 动画1 reads the reference at a sprint, this one keeps every beat the reference has: the
 * play head sinks into a *recessed* well rather than a flat disc, the drop lands and stands as
 * a pool before it breaks up, the bubbles reassemble into the triangle inside the well, and the
 * burst throws droplets clear of the cloud to drift and fade. The reference runs 5.04s; this is
 * the same choreography at 0.4x, which is where it still reads at launch speed.
 */
internal object SplashTwo : SplashChoreography {
    ...
    override fun DrawScope.drawMark(nowMs: Float) = drawCloudWellMark(nowMs)

    override fun wordmark(nowMs: Float) = easeOutCubic(span(nowMs, WordmarkStartMs, WordmarkMs))

    override fun tagline(nowMs: Float) = easeOutCubic(span(nowMs, TaglineStartMs, TaglineMs))
}

private fun DrawScope.drawCloudWellMark(nowMs: Float) {
    val d = size.minDimension
    val intro = easeOutCubic(span(nowMs, 0f, IntroMs))
    val breathing = sin(nowMs / SplashTwo.durationMs * PiF) * 0.007f
...
private fun cloudCompression(nowMs: Float, lagMs: Float = 0f): Float {
    val t = nowMs - lagMs
...
private fun DrawScope.drawWellStack(d: Float, nowMs: Float) {
    ...
    val pool = smooth(span(nowMs, ImpactMs, PoolMs)) *
 ...
 */
private fun DrawScope.drawWell(d: Float, centre: Offset, radius: Float, solid: Float) {
    val hollow = 1f - solid
...
private fun DrawScope.drawPool(centre: Offset, radius: Float, level: Float) {
    ...
    clipPath(wellPath) {
        ...
        val halfWidth = radius * sqrt((1f - ((surfaceY - centre.y) / radius).let { it * it })
            .coerceAtLeast(0f))
        drawOval(
...
private fun DrawScope.drawFallingDrop(d: Float, centre: Offset, nowMs: Float) {
 ...
 */
private fun DrawScope.drawWellBubbles(d: Float, centre: Offset, radius: Float, nowMs: Float) {
    val boil = smooth(span(nowMs, BoilStartMs, BoilMs))
    val gather = smooth(span(nowMs, GatherStartMs, GatherMs))
    val alpha = smooth(span(nowMs, BoilStartMs, BubbleFadeInMs)) *
...
private fun DrawScope.drawPlayHead(d: Float, centre: Offset, nowMs: Float) {
...
private fun DrawScope.drawPlayGlyph(d: Float, centre: Offset, scale: Float, alpha: Float) {
...
private fun DrawScope.drawDiscGlint(d: Float, centre: Offset, radius: Float, nowMs: Float) {
 ...
 */
private fun DrawScope.drawSpray(d: Float, nowMs: Float) {
    ...
    repeat(SprayCount) { index ->
        ...
        val position = Offset(
            ...
            y = origin.y + sin(angle) * reach * travelled * 0.82f +
...
private fun DrawScope.drawWaterDrop(
    ...
    val path = Path().apply {
        ...
        cubicTo(
            centre.x - rx * 0.24f, centre.y - ry * 0.72f,
            centre.x - rx * 0.80f, centre.y - ry * 0.18f,
            centre.x - rx * 0.80f, centre.y + ry * 0.30f,
        )
        cubicTo(
            centre.x - rx * 0.80f, centre.y + ry * 1.10f,
            centre.x + rx * 0.80f, centre.y + ry * 1.10f,
            centre.x + rx * 0.80f, centre.y + ry * 0.30f,
        )
        cubicTo(
...
private fun DrawScope.drawBubble(centre: Offset, radius: Float, alpha: Float) {
...
private val TriangleFill: List<Pair<Float, Float>> = run {
    val columns = 6
...
private const val TaglineStartMs = 1_400f
private const val TaglineMs = 340f

/** Derived, not typed in, so retuning the fall cannot desynchronise the crown from the drop. */
private val CrownContactMs: Float = run {
