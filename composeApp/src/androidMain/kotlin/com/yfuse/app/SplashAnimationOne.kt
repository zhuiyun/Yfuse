...
/**
 * 动画1 — the drop falls into the cloud and shatters into the play head.
 *
 * A tight 1.9s read of the reference: the old play head collapses, a drop falls under gravity,
 * the cloud takes the hit as a damped squash, and the burst gathers back into the triangle.
 */
internal object SplashOne : SplashChoreography {
    ...
    override fun DrawScope.drawMark(nowMs: Float) = drawCloudPlayerMark(nowMs)
...
private fun DrawScope.drawCloudPlayerMark(nowMs: Float) {
    ...
    val breathing = sin(nowMs / SplashOne.durationMs * PiF) * 0.006f
...
private fun DrawScope.drawWaterTimeline(d: Float, nowMs: Float) {
    val center = Offset(d * 0.50f, d * DiscY)
    ...
    val fragmentAlpha = smooth(span(nowMs, ImpactMs, FragmentInMs)) *
...
private fun DrawScope.drawWaterDrop(
    ...
    val path = Path().apply {
        ...
        cubicTo(
            center.x - rx * 0.25f, center.y - ry * 0.70f,
            center.x - rx * 0.78f, center.y - ry * 0.18f,
            center.x - rx * 0.78f, center.y + ry * 0.30f,
        )
        cubicTo(
            center.x - rx * 0.78f, center.y + ry * 1.08f,
            center.x + rx * 0.78f, center.y + ry * 1.08f,
            center.x + rx * 0.78f, center.y + ry * 0.30f,
        )
        cubicTo(
...
private const val TaglineStartMs = 1_050f
private const val TaglineMs = 350f
