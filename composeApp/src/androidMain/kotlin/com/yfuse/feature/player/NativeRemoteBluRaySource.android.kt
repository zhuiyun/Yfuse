 ...
 *
 * The native side sees only [readBlocksNative] and [closeNativeSource]. Every HTTP request resolves
 * the currently saved server route/token again, so a route failover or renewed session does not leave
 * a 100 GiB movie tied to stale credentials. Tokens are headers, never part of the source URL.
 *
...
class NativeRemoteBluRayBlockSource(
     ...
     * deterministic if a future navigation/event thread asks for metadata while playback is reading.
     */
    @Synchronized
