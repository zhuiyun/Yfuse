...
class AmbientLightTransitionTest {
    ...
    @Test
    fun transition_is_paced_and_reaches_the_exact_target() =
        runTest {
    ...
    @Test
    fun cancelling_visibility_stops_all_future_publications() =
        runTest {
            var frames = 0
            val job =
                launch {
    ...
    @Test
    fun zero_duration_publishes_only_the_target_without_starting_a_clock() =
        runTest {
            ...
            runAmbientLightTransition(
                ...
                cool,
                0L,
                { error("No clock when animations are disabled") },
            ) { frames += it }
