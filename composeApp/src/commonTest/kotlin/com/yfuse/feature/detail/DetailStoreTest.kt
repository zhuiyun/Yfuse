...
class DetailStoreTest {
    ...
    @Test
    fun quality_sorting_does_not_replace_the_current_server_as_the_default_selection() =
        runTest {
            val lowQualityCurrent =
                MOVIE_ONE
    ...
    @Test
    fun resource_selection_retries_transient_connection_failures_then_succeeds() =
        runTest {
    ...
    @Test
    fun resource_selection_stops_after_bounded_network_retries() =
        runTest {
            val attempts = AtomicInteger()
            val store =
                movieStore(
    ...
    @Test
    fun resource_selection_does_not_retry_a_cloudflare_access_block() =
        runTest {
            val attempts = AtomicInteger()
            val store =
                movieStore(
    ...
    @Test
    fun tapping_a_pending_resource_again_queues_play_after_resolution() =
        runTest {
    ...
    @Test
    fun newer_failed_resource_switch_keeps_the_last_committed_source() =
        runTest {
    ...
    @Test
    fun stalled_resource_switch_has_a_wall_clock_deadline() =
        runTest {
            val started = CompletableDeferred<Unit>()
            val store =
                movieStore(
    ...
    @Test
    fun favorite_state_survives_switching_to_another_episode() =
        runTest {
            val started = CompletableDeferred<Unit>()
            val release = CompletableDeferred<Unit>()
            val store =
    ...
    @Test
    fun cross_server_player_selection_commits_its_exact_episode_and_version() =
        runTest {
            val store =
                seriesStore(
    ...
    @Test
    fun server_switch_during_episode_resolution_keeps_the_newly_selected_coordinate() =
        runTest {
            val started = CompletableDeferred<Unit>()
            val release = CompletableDeferred<Unit>()
            val store =
    ...
    @Test
    fun stale_episode_response_cannot_commit_while_server_switch_is_resolving() =
        runTest {
    ...
    @Test
    fun episode_tap_during_server_switch_restarts_resolution_and_plays_new_coordinate() =
        runTest {
    ...
    @Test
    fun cross_server_episode_resolution_retries_and_keeps_the_same_coordinate() =
        runTest {
    ...
    @Test
    fun source_without_the_current_episode_does_not_fall_back_to_next_up() =
        runTest {
            var secondNextUpCalls = 0
            val store =
                seriesStore(
    ...
    @Test
    fun exhausted_season_load_keeps_the_previous_episode_directory() =
        runTest {
            val attempts = AtomicInteger()
            val store =
                seriesStore(
    ...
    private companion object {
        ...
        const val MOVIE_THREE =
            """{"Id":"m3","Name":"电影","Type":"Movie",""" +
        ...
        const val ALT_EPISODE_NINE =
            """{"Id":"ae9","Name":"第九集","Type":"Episode",""" +
                """"SeriesId":"s2","SeriesName":"剧集","ParentIndexNumber":1,""" +
                """"IndexNumber":9,"SeasonId":"aseason1","MediaSources":[""" +
                """{"Id":"aev9","Name":"备库第九集","MediaStreams":[""" +
                """{"Type":"Video","Height":720}]}]}"""

        const val ALT_EPISODE_TWO =
            """{"Id":"ae2","Name":"第二集","Type":"Episode",""" +
                """"SeriesId":"s2","SeriesName":"剧集","ParentIndexNumber":1,""" +
